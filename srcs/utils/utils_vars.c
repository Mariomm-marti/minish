...
/*   Updated: 2022/06/17 00:02:16 by mmartin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	environ_to_heap(void)
{
	...
	while (*(environ + envlen))
		envlen++;
	g_environ_heap = (char **)malloc(sizeof(char *) * (envlen + 1));
	i = 0;
	while (i < envlen)
	{
		*(g_environ_heap + i) = ft_strdup(*(environ + i));
		i++;
	}
	*(g_environ_heap + i) = NULL;
}
